...
function viewStudentStats(id, toggle) {
	...
	//if toggle = 0 hide and set height to 0
	if (toggle == 0) {
		document.getElementById(id).style.visibility = "hidden";
		document.getElementById(id).style.height = 0;
	}
	//if toggle = 1 show and set height to auto
	if (toggle == 1) {
...
function loadStatsPage() {
		...
		{//Outer for loop iterates over each row
			...
			{
				if (col_counter = 2) {
					buff_score = table.rows[row_counter].cells[col_counter].innerHTML;
					buff_score = parseInt(buff_score);
				}
				if (col_counter = 3) {
					opp_score = table.rows[row_counter].cells[col_counter].innerHTML;
					opp_score = parseInt(opp_score);
				}
				if (col_counter = 4) {
					if (opp_score > buff_score) {
						table.rows[row_counter].cells[col_counter].innerHTML = "opps win";
						losses++;
					}
					else if (buff_score > opp_score) {
						table.rows[row_counter].cells[col_counter].innerHTML = "buffs win";
						wins++;
					}
				}
...
function loadPlayersPage () {
	...
	player.innerHTML += "<a href='#' class='dropdown-item' onClick='switchPlayers(0)'>John Doe</a>";
	player.innerHTML += "<a href='#' class='dropdown-item' onClick='switchPlayers(1)'>James Smith</a>";
	player.innerHTML += "<a href='#' class='dropdown-item' onClick='switchPlayers(2)'>Samuel Phillips</a>";
	player.innerHTML += "<a href='#' class='dropdown-item' onClick='switchPlayers(3)'>Robert Myers</a>";
}
